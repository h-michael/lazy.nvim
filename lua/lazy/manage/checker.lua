...
function M.fast_check(opts)
  ...
  for _, plugin in pairs(Config.plugins) do
    ...
    if plugin._.installed and not (plugin.pin or plugin._.is_local) then
      ...
      -- Without an age constraint, the age-ignoring target is identical to
      -- `target`, so skip the extra resolution pass entirely.
      local raw_ok, raw_target = ok, target
      if Git.get_minimum_release_age(plugin) then
        raw_ok, raw_target = pcall(Git.get_target, plugin, true)
      end
